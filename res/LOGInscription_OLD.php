	...
	<body>
		...
		if(isset($_POST['valider'])){
			...
			if(!isset($_POST['pseudo'],$_POST['mdp'],$_POST['mail'])){
				...
				if(strlen($_POST['pseudo'])<5 or strlen($_POST['pseudo'])>50){
					...
					//d'une longueur de 1 min et 50 max
					echo "Le pseudo est incorrect, doit contenir seulement des lettres minuscules et/ou des chiffres, d'une longueur minimum de 1 caractère et de 50 maximum.";
					...
					if(strlen($_POST['mdp'])<5 or strlen($_POST['mdp'])>50){
						echo "Le mot de passe doit être d'une longueur minimum de 5 caractères et de 50 maximum.";
					} else {
						...
						if(!preg_match("#^[a-z0-9_-]+((\.[a-z0-9_-]+){1,})?@[a-z0-9_-]+((\.[a-z0-9_-]+){1,})?\.[a-z]{2,30}$#i",$_POST['mail'])){
							...
							if(strlen($_POST['mail'])<7 or strlen($_POST['mail'])>50){
								...
								if(!$mysqli) {
									...
									if(mysqli_num_rows(mysqli_query($mysqli,"SELECT * FROM photolabmembre WHERE pseudo='$Pseudo'"))!=0){//si mysqli_num_rows retourne pas 0
										...
										if(mysqli_query($mysqli,"INSERT INTO photolabmembre SET pseudo='$Pseudo', mdp='$Mdp', mail='$Mail'")){
											echo "Inscrit avec succès! Vous pouvez vous connecter: <a href='index.php'>Cliquez ici</a>.";
